...
def fpga(tasks, nf, file_fpga, file_data, sti, stw, task_int, task_length, tcfg, tslr, task_name_column, data_column, period_column, cu_column, xclbin_loc_column, xclbin_com_column, in_data_com_column, in_data_loc_column, host_loc_column, host_com_column, ii_column):
   task_set=tasks
   start=0
   end=0
   eti=0
   ...
   for i in range(sti, task_length): # for a single task
         ...
         if(j>=start) and j<(start+tcfg):
           ...
           if xclbin_flag==0:
            ...
            if(end-start-tcfg==w):
             ...
             file_fpga.write("\n"+host_com+" ./"+host_loc+host_name+" "+xcl_com+" ./"+xcl_loc+xclbin_name+" "+data_com+" "+split_data_name[0]+str(data_size)+split_data_name[1]+" -id "+str(nf))
      ...
      if rc<=tcfg and rc>=0:
        ...
        break
      #if rc<0:
      if rc<0 and rc1 >tcfg+int(task_set[i+1][ii_column]):
        eti=i
        etw=rc1-tcfg+stw1
        break        
      if rc<0 and rc1 <=tcfg+int(task_set[i+1][ii_column]):
...
def fpga_script(tasks, tcfg, tslr, nf, task_int, length, task_name_column, data_column, period_column, cu_column, xclbin_loc_column, xclbin_com_column, in_data_com_column, in_data_loc_column, host_loc_column, host_com_column, ii_column):
  ...
  for i in range(0, nf): # for nf FPGAs
    ...
    print("*********FPGA"+str(i)+"***********")
    sti, stw=fpga(tasks, i, file_fpga, file_data, int(sti), int(stw), task_int, length, tcfg, tslr, task_name_column, data_column, period_column, cu_column, xclbin_loc_column, xclbin_com_column, in_data_com_column, in_data_loc_column, host_loc_column, host_com_column, ii_column)
    if sti==0 and stw==0:
     print("***************************")
